...
public record SyncOverlaysPayload(Map<BlockPos, NbtCompound> tags) implements CustomPayload {
    public static final CustomPayload.Id<SyncOverlaysPayload> ID = new CustomPayload.Id<>(Identifier.of(Underlay.MOD_ID, "sync_overlays"));
    ...
    public static final PacketCodec<PacketByteBuf, SyncOverlaysPayload> CODEC = PacketCodec.ofStatic(
        (buf, payload) -> {
			...
			for (Map.Entry<BlockPos, NbtCompound> e : payload.tags().entrySet()) {
