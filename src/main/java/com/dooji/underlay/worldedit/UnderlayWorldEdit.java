...
public final class UnderlayWorldEdit {
    ...
    public static void removeOverlay(ChangeSet changeSet, ServerWorld world, BlockPos pos) {
    ...
    private static void placeOverlay(Extent extent, BlockVector3 position, BlockState overlay) {
        ...
        World adaptedWorld = FabricAdapter.adapt(editSession.getWorld());
        if (!(adaptedWorld instanceof ServerWorld world)) {
            return;
        }
    ...
    private static void setOverlay(ServerWorld world, BlockPos pos, BlockState overlay) {
        if (overlay == null) {
    ...
    private record OverlayChange(ServerWorld world, BlockPos pos, BlockState oldOverlay, BlockState newOverlay) implements Change {
        @Override
