...
public final class UnderlayWorldEdit {
    ...
    private static BlockState getOverlay(Extent extent, BlockVector3 position) {
        ...
        if (extent instanceof BlockTransformExtent transformExtent) {
            ...
            return overlay == null ? null : ForgeAdapter.adapt(BlockTransformExtent.transform(ForgeAdapter.adapt(overlay), transformExtent.getTransform()));
        }

        if (extent instanceof EditSession editSession) {
            ServerLevel world = ForgeAdapter.adapt(editSession.getWorld());
            return UnderlayManager.getOverlaysFor(world).get(ForgeAdapter.toBlockPos(position));
        }
