package com.dooji.underlay.main.worldedit;
...
public final class UnderlayWorldEdit {
    ...
    private static BlockState getOverlay(Extent extent, BlockVector3 position) {
        ...
        if (extent instanceof BlockTransformExtent transformExtent) {
            ...
            return overlay == null ? null : NeoForgeAdapter.adapt(BlockTransformExtent.transform(NeoForgeAdapter.adapt(overlay), transformExtent.getTransform()));
        }

        if (extent instanceof EditSession editSession) {
            ServerLevel world = NeoForgeAdapter.adapt(editSession.getWorld());
            return UnderlayManager.getOverlaysFor(world).get(NeoForgeAdapter.toBlockPos(position));
        }
